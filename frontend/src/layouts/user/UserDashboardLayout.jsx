...
const UserDashboardLayout = () => {
          ...
          <Box key={group.groupTitle} sx={{ mb: 3 }}>
            ...
            <List disablePadding>
              {group.items.map((item) => {
                const isActive = location.pathname === item.path || (location.pathname === '/user' && item.path === '/user/dashboard' && item.text === 'Dashboard Home');
                return (
  ...
  const activeTitle = allItems.find(item => item.path === location.pathname)?.text || 'Dashboard';
